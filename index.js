...
async function run() {
    try {
        ...
        const dollCollection = client.db('educationalDoll').collection('dolls');
...
app.get('/', (req, res) => {
    res.send('Educational doll is running')
})

app.listen(port, () => {
    console.log(`Educational doll is running on port ${port}`)
})